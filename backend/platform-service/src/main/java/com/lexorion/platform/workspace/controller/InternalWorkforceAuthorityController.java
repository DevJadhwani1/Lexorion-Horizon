...
import com.lexorion.platform.workspace.dto.*;
...
public class InternalWorkforceAuthorityController {
    ...
    @PostMapping("/{workspaceKey}/user-resolution") public UserResolutionResponse resolveUser(@PathVariable @Pattern(regexp="^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$") String workspaceKey, @Valid @RequestBody UserResolutionRequest request) {
        ...
        var context = contexts.get().orElseThrow();
        if (context.membershipRole() != OrganizationRole.OWNER && context.membershipRole() != OrganizationRole.ADMIN) throw new AccessDeniedException("Organization administrator authority is required");
    ...
    private InternalWorkforceContextResponse response(OrganizationWorkspace workspace, com.lexorion.platform.tenantaccess.context.TenantAccessContext context) {
    ...
    private OrganizationWorkspace workforceWorkspace(String key) {
        ...
        if (!"workforce".equals(workspace.getProduct().getKey())) throw new AccessDeniedException("Workforce workspace is required");
        return workspace;
    }
}
